...
class Solution:
    def countAndSay(self, n: int) -> str:
        ...
        if n==1:
            return "1"
        return "".join([str(len(item))+item[0] for item in [item[0] for item in re.findall(r"((.)\2*)", s.countAndSay(n-1))]])


"""
Runtime: 36 ms, faster than 42.44% of Python3 online submissions for Count and Say.
