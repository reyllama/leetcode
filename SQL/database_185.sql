...
FROM (SELECT Name as Employee, Salary, DepartmentId, DENSE_RANK() OVER (PARTITION BY DepartmentId ORDER BY Salary DESC) as sal_rank FROM Employee) a
INNER JOIN (SELECT Id as dId, Name as Department FROM Department) b
...
'''
Runtime: 1314 ms, faster than 71.60% of Oracle online submissions for Department Top Three Salaries.
